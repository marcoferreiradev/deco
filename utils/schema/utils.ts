...
const getSchemaFromFunctionExport = (acceptedTypes: string[]) =>
  withErrorPath(async (path: string) => {
    const nodes = await denoDoc(path);
    const node = findExport("default", nodes);
    ...
    if (tsType.kind !== "typeRef" || tsType.typeRef.typeName in acceptedTypes) {
      throw new Error(`Default export needs to any of type ${acceptedTypes}`);
    }

    const [propType = null, returnType = null] =
...
export const getSchemaFromLoaderExport = getSchemaFromFunctionExport([
  "LoaderFunction",
  "MatchFunction",
  "EffectFunction",
]);

export const getSchemaFromWorkflowExport = getSchemaFromFunctionExport([
  "WorkflowFunction",
]);
