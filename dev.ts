...
async function generateDevManifestData(dir: string): Promise<DevManifestData> {
  const [manifestFile, sections, functions, workflows] = await Promise.all([
    collect(dir),
    collectSections(dir),
    collectFunctions(dir),
    colelctWorkflows(dir),
  ]);
  const schemas = await extractAllSchemas(sections, functions, workflows);
...
export async function generate(directory: string, manifest: DevManifestData) {
  const { routes, islands, sections, functions, workflows, schemas } = manifest;
  ...
  const output = `// DO NOT EDIT. This file is generated by deco.
    ...
    ${workflows.map(templates.workflows.imports).join("\n")}
    ...
    const manifest: DecoManifest = {
      ...
      workflows: {${workflows.map(templates.workflows.obj).join("\n")}},
...
function manifestDataEquals(a: DevManifestData, b: DevManifestData) {
  return (
    ...
    arraysEqual(a.workflows, b.workflows)
  );
}
...
async function colelctWorkflows(dir: string): Promise<string[]> {
  const workflowDir = join(dir, "./workflows");

  const workflowNames = await collectFilesFromDir(workflowDir);
...
const templates = {
  ...
  workflows: {
    imports: (file: string, i: number) =>
      `import * as $$$$$${i} from "./workflows${file}";`,
    obj: (file: string, i: number) => `"./workflows${file}": $$$$$${i},`,
  },
};
...
async function extractAllSchemas(
  ...
  const functionSchemasAsArray = await Promise.all(
    functions.map(async (functionName) => {
      ...
      return [path, functionSchema] as const;
    })
  );
  ...
  const sectionSchemasAsArray = await Promise.all(
    sections.map(async (section) => {
      ...
      return [path, sectionSchema] as const;
    })
  );

  const workflowSchemasAsArray = await Promise.all(
    workflows.map(async (workflow) => {
      const path = `./workflows${workflow}`;
      const workflowSchema = await getSchemaFromWorkflowExport(path);

      return [path, workflowSchema] as const;
    })
  );
